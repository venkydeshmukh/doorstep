...
exports.count = function(req, res) {
  ...
  if(req.query){
    ...
    Product.find(q).where('merchantId').equals(merchantId).count().exec(function (err, count) {
...
//Get list of products for particular merchant
exports.merchant = function(req, res) {
  ...
  if(req.query){
    ...
    var select = isJson(req.query.select);
    // setTimeout(function(){
      Product.find(q).where('merchantId').equals(merchantId).limit(req.query.limit).skip(req.query.skip).sort(sort).select(select).exec(function (err, products) {
        if(err) { return handleError(res, err); }
        return res.status(200).json(products);
      });
    // },2000);
    ...
    Product.find(function (err, products) {
      if(err) { return handleError(res, err); }
      return res.status(200).json(products);
    }).where('merchantId').equals(merchantId);
  }
};
